...
//            map: this.spriteTextureSignal,
    ...
    Axon.prototype.getControlPoint = function (v1, v2) {
        ...
        var theta = THREE.Math.randFloat(0, Math.PI * 16);
    ...
    function NeuralNetwork() {
        ...
        // settings
        this.verticesSkipStep = 16;	//2
        this.maxAxonDist = 16;	//8 --
        this.maxConnectionPerNeuron = 6;	//6

        this.currentMaxSignals = 500;
...
//            map: this.spriteTextureNeuron,
    ...
    NeuralNetwork.prototype.initNeuralNetwork = function () {
        ...
        var loadedObject = getTextPointCloud();
        loadedMesh = loadedObject.children[0];

        loadedMeshVertices = loadedMesh.geometry.vertices;

        // render loadedMesh
        loadedObject.material = new THREE.MeshBasicMaterial({
            transparent: true,
            opacity: 0.05,
            depthTest: false,
            color: 0x0088ff,
    ...
    cameraCtrl.object.position.z = 75;
    cameraCtrl.update();
...
//    gui_settings.add(neuralNet, 'neuronOpacity', 0, 1.0).name('Neuron Opacity');
    ...
    function getTextPointCloud() {
        ...
        if(fragment !== undefined && fragment.length > 5) {
            var fragMd5 = fragment.substring(fragment.length - 4, fragment.length);
            var textb64 = fragment.substring(1, fragment.length - 4);

            if(md5(atob(textb64)).substring(0,4) == fragMd5) {
                text = atob(textb64);
                console.log("emacsem przez sendmail")
            } else {
                console.log("potrojna sciana ognia")
            }
        }
